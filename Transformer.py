...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=512, N: int=6, h: int=8, dropout: float=0.1, d_ff: int=2048) -> Transformer:
    ...
    encoder_blocks = []
    for _ in range(N):
        encoder_self_attention_block = multi_head_atten(d_model, h, dropout)
        feed_forward_block = feed_forrward_nn(d_model, d_ff, dropout)
    ...
    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = multi_head_atten(d_model, h, dropout)
        decoder_cross_attention_block = multi_head_atten(d_model, h, dropout)
        feed_forward_block = feed_forrward_nn(d_model, d_ff, dropout)
